...
import LibrarianRecommendGalleryComponent from "./LibrarianRecommendGalleryComponent";
...
const getFetchData = async ({ pageParam = 1, queryKey }) => {
    ...
    const filteredGroup = data.librarianRecommend.find(group => group.code === selC1);
...
export default function LibrarianRecommendGallery() {
    ...
    const {
        ...
        queryKey: ["librarianRecommend", selC1],
        ...
        <div className={`relative !pt-0`}>
            <div className="bg-white z-10 py-3 fixed top-20 w-full shadow-shadowBottom ">
                <div className={`flex flex-wrap`}>
                    {data2.librarianCategory.map((item, idx) => (
                        <button
                            key={idx}
            ...
            {allItems.length > 0 ? (
                <ul className="items-stretch w-full pt-20">
                    {allItems.map((item, idx) => (
                        <li key={idx} className="h-full flex cursor-pointer py-4">
                            <LibrarianRecommendGalleryComponent item={item}/>
                        </li>
                    ))}
                </ul>
            ) : (
